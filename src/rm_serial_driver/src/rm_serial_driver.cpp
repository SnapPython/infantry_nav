    ...
    while (rclcpp::ok()) {
        // 发送数据的代码
         try{ 
          ...
          // }
          RCLCPP_INFO(get_logger(), "send data vx: %f vy: %f", sendpacket.nav_vx, sendpacket.nav_vy);
    } catch (const std::exception & ex) {
...
void RMSerialDriver::navsendData(const geometry_msgs::msg::Twist& cmd_vel)
{
  ...
  // try {
    sendpacket.naving = 1;
    sendpacket.nav_vx = cmd_vel.linear.x*5;
    sendpacket.nav_vy = cmd_vel.linear.y*5;
